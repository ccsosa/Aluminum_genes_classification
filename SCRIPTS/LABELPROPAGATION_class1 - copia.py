...
results.to_csv("D:/REPO_GITHUB/ALUMINUM_GENES_CLASSIFICATION/RESULTS/WEIGHTED/out_pred_class1LP.csv")
...
X_train, X_test, y_train, y_test = train_test_split(X3_to, y3_to, test_size=0.2, random_state = 0)
...
cv_results = cross_validate(label_prop_model, X_train, y_train, cv=stratified_cv)

from sklearn.model_selection import cross_val_predict
y_test_pred = cross_val_predict(label_prop_model,X_test, y_test,cv=stratified_cv)
...
#Calculo de las metricas Precision, Recall, F1-score 
from sklearn.metrics import classification_report
report_LP = classification_report(y_test,y_test_pred)
print(report_LP)


y_pred_LP = cv_results.predict(X_test)
label_prop_model.score(X_test, y_test)
...
score = accuracy_score(y_test, y_pred_LP)
...
#results.to_csv("D:/TESIS_PHD/RICENETPPI/out_pred.csv")
results.to_csv("D:/REPO_GITHUB/ALUMINUM_GENES_CLASSIFICATION/RESULTS/UNWEIGHTED/out_pred_class1LP.csv")
